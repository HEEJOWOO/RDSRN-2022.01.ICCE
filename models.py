...
class RecursiveBlock(nn.Module):
    def __init__(self,growth_rate,U,wn):
        super(RecursiveBlock, self).__init__()
        self.U = U
        self.G = growth_rate
        wn = wn
        self.DWCB = DWCB(self.G, wn=wn)
    def forward(self, sfe2):
        global Recursive_feature
        x=sfe2
        local_features = []
        ...
        Recursive_feature = torch.cat(local_features, 1)
        return Recursive_feature
        
#High Frequency Residual Refinement Block        
class HRB(nn.Module):
...
class RDSRN(nn.Module):
    def __init__(self, scale_factor, num_channels, num_features, growth_rate, U):
        super(RDSRN, self).__init__()
        ...
        #1.Shallow Feature Extraction Step
        self.sfe1 = wn(nn.Conv2d(num_channels, self.G0, kernel_size=3, padding=3 // 2))
        self.sfe2 = wn(nn.Conv2d(self.G0, self.G0, kernel_size=3, padding=3 // 2))
        
        #2.a feature extraction step with a recursive block
        self.RecursiveBlock = nn.Sequential(*[RecursiveBlock(self.G,self.U,wn=wn)])
        
        self.gff = wn(nn.Conv2d(self.G * self.U, self.G0, kernel_size=1))
        ...
        #4.High Frequency Residual Refinement Block & Reconstruction
        self.HRB = HRB(self.G0,wn)
    ...
    def forward(self, x):
        ...
        Recursive_feature= self.RecursiveBlock(sfe2)
        x = self.gff(Recursive_feature) + sfe1
        x = self.gff_2(x)
